...
/**
 * A version of {@link BadPS4} with {@link Trigger} factories for command-based.
 * 
 * Trigger factories for the L2 and R2 buttons have been removed, as they are treated as analog input.
 *
 * @see BadPS4
 * @see CommandPS4Controller
 */
public class CommandBadPS4 extends CommandGenericHID {
    private final BadPS4 m_hid;
     ...
     */
    public CommandBadPS4(int port) {
        super(port);
        m_hid = new BadPS4(port);
    }
     ...
     * Constructs an event instance around the L2 button's digital signal.
     *
     * @param threshold the threshold before activating (default 0.5)
     * @param loop the event loop instance to attach the event to.
     * @return an event instance representing the L2 button's digital signal attached to the given
     *     loop.
     */
